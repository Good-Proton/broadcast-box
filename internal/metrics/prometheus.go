...
var (
	...
	whepSessionPacketsWritten = promauto.NewGaugeVec(
		...
		[]string{"stream_key", "session_id", "current_layer"},
	)
...
func UpdateMetrics() {
	...
	for _, status := range statuses {
		streamFirstSeenEpoch.WithLabelValues(status.StreamKey).Set(float64(status.FirstSeenEpoch))
		streamAudioPacketsReceived.WithLabelValues(status.StreamKey).Set(float64(status.AudioPacketsReceived))

		for _, video := range status.VideoStreams {
			streamVideoPacketsReceived.WithLabelValues(status.StreamKey, video.RID).Set(float64(video.PacketsReceived))

			if !video.LastKeyFrameSeen.IsZero() {
				streamVideoLastKeyFrameSeen.WithLabelValues(status.StreamKey, video.RID).Set(float64(video.LastKeyFrameSeen.Unix()))
			}
		}

		activeWhepSessionsCount.WithLabelValues(status.StreamKey).Set(float64(len(status.WHEPSessions)))

		for _, session := range status.WHEPSessions {
			whepSessionPacketsWritten.WithLabelValues(status.StreamKey, session.ID, session.CurrentLayer).Set(float64(session.PacketsWritten))
			whepSessionSequenceNumber.WithLabelValues(status.StreamKey, session.ID, session.CurrentLayer).Set(float64(session.SequenceNumber))
			whepSessionTimestamp.WithLabelValues(status.StreamKey, session.ID, session.CurrentLayer).Set(float64(session.Timestamp))
		}
	}
}
